...
class MainWindow(tk.Tk):
    ...
    def __init__(self):
        ...
        tk.Button(text="Выбрать файл", command=self.readFile, font=font_arial).\
            grid(row=3, column=0, pady=10, padx=10, sticky='w')
        tk.Button(text="График", command=self.graph, font=font_arial).\
            grid(row=3, column=2, pady=10, padx=10, sticky='e')
        tk.Button(text="Расчет", command=self.count, font=font_arial).\

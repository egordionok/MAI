...
def Newton(coefs_):
    ...
    if len(coefs_) > 2:  # запускаем рекурсию
        extremums = Newton(differential(coefs_))
    ...
    for i in range(len(extremums) - 1):
        ...
        if f(lower_bound, coefs_) == 0:
            ...
            if f(lower_bound, coefs_) * f(upper_bound, coefs_) > 0:
                continue
            x_prev = (lower_bound + upper_bound) / 2
            dif_coefs = differential(coefs_)
            x = x_prev - f(x_prev, coefs_)/f(x_prev, dif_coefs)

            i = 0
            while abs(x - x_prev) >= 0.0001 and i < 1000:
                x, x_prev, i = x - f(x, coefs_)/f(x, dif_coefs), x, i + 1
...
window = tk.Tk()
window.title('Поиск корней методом Ньютона')
